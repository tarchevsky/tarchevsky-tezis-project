...
Аналог популярного контейнера .container, но задаёт глобальный контейнер, определяющий размеры шапки, контентной части и подвала. Задаётся через grid-template-rows и grid-template-columns. grid-template-columns содержит переменную $container, которая задаётся в ``./src/styles/helpers/_variables.scss`` и определяет **базовую ширину сайта**.
